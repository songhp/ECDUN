...
class checkpoint():
    ...
    def save_results(self, filename, save_list, scale):
        ...
        for v, p in zip(save_list, postfix):
            filename1 = '{}/results/{}/'.format(self.dir, p)
            if not os.path.exists(filename1):
                os.mkdir(filename1)
            filename1 = '{}/{}/'.format(filename1, self.args.testset)
            if not os.path.exists(filename1):
                os.mkdir(filename1)
            filename1 = '{}/X{}/'.format(filename1, scale)
            if not os.path.exists(filename1):
                os.mkdir(filename1)
            normalized = v[0].data.mul(255 / self.args.rgb_range)
            ndarr = normalized.byte().permute(1, 2, 0).cpu().numpy()
            imageio.imsave('{}/{}.png'.format(filename1, filename), ndarr)
